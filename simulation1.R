...
W<-rnorm(n,mean=(0.67*C+0.25*L+0.25*A+0.25*M),sd=0.5)
Y<-rnorm(n,mean=(0.122+0.45*A+0.35*L+0.5*C+0.45*W+0.25*M+0.1*A*M+0.1*A*W+0.1*M*W+0.5*A*M*W),sd=0.5)
data <-as.data.frame(cbind(A=A,Y=Y,C=C,M=M,W=W,L=L))

#抽象函数
C <- rnorm(n)
A<-rbinom(n,1,prob=0.5)
L.fun <- function(C,A)rnorm(n,mean=(0.5+0.37*C+1.3*A),sd=0.5)
M.fun <- function(A,L,C)rnorm(n,mean=(0.53*L+0.35*A+0.26*C-0.1),sd=0.5)
W.fun <- function(A,L,C,M)rnorm(n,mean=(0.67*C+0.25*L+0.25*A+0.25*M),sd=0.5)
Y.fun <- function(A,L,C,M,W)rnorm(n,mean=(0.122+0.45*A+0.35*L+0.5*C+0.45*W+0.25*M+0.1*A*M+0.1*A*W+0.1*M*W+0.5*A*M*W),sd=0.5)
...
write.table(sim1,paste0(path_data,"sim1.csv"),sep = ",",col.names = TRUE,row.names = FALSE)
