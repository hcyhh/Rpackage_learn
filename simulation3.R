...
L <- rnorm(n,mean=(0.5+0.37*C+1.3*A),sd=0.5)
M<-rnorm(n,mean=(0.53*L+0.35*A+0.26*C-0.1),sd=0.5)
W<-rnorm(n,mean=(0.67*C+0.25*L+0.25*A),sd=0.5)
Y<-rnorm(n,mean=(0.122+0.45*A+0.35*L+0.5*C+0.45*W+0.25*M+0.1*A*M+0.1*A*W),sd=0.5)
data <-as.data.frame(cbind(A=A,Y=Y,C=C,M=M,W=W,L=L))

#抽象函数
C <- rnorm(n)
A<-rbinom(n,1,prob=0.5)
L.fun <- function(C,A)0.5+0.37*C+1.3*A
M.fun <- function(A,L,C)0.53*L+0.35*A+0.26*C-0.1
W.fun <- function(A,L,C,M)0.67*C+0.25*L+0.25*A
Y.fun <- function(A,L,C,M,W)0.122+0.45*A+0.35*L+0.5*C+0.45*W+0.25*M+0.1*A*M+0.1*A*W
...
m_form <- M ~ C * A
w_form <- W ~ C * A 
y_form <- Y ~ C*A+M+W+A*W+A*M+L

#链接函数
m1.family <- gaussian(link = "identity")
m2.family <- gaussian(link = "identity")
...
decompz <- function(treatment, pre_cov, zmodels, y_form, m_form,
  ...
  decomp <- function(object){
    ...
    if (object$m_model1$family$link != "identity"){
      ...
      w0 <- mean(predict.glm(object$m_model2, data.frame(C=newdata0[["C"]],A=newdata0[["A"]]), type = "response"))
      ...
      w1 <- mean(predict.glm(object$m_model2, data.frame(C=newdata1[["C"]],A=newdata1[["A"]]), type = "response"))
    ...
    NDE <-  ((a1-a0)*(beta2+beta6*(pred0)+beta7*w0))
    
    CDE <- ((a1-a0)*(beta2+beta6*0+beta7*0))
    
    INT.re.am <- ((a1-a0)*(beta2+beta6*(pred0)+beta7*0))-((a1-a0)*(beta2+beta6*0+beta7*0))
    INT.re.aw <- ((a1-a0)*(beta2+beta6*0+beta7*w0))-((a1-a0)*(beta2+beta6*0+beta7*0))
    INT.re.amw <- (((a1-a0)*(beta2+beta6*(pred0)+beta7*(w0))))-
      ((a1-a0)*(beta2+beta6*(pred0)+beta7*0))-
      ((a1-a0)*(beta2+beta6*0+beta7*w0))+
      ((a1-a0)*(beta2+beta6*0+beta7*0))
    INT.ref <- INT.re.am+INT.re.aw+INT.re.amw
    #CDE+INT.ref
    
    
     NIE <- (beta4*(pred1-pred0)+beta5*(w1-w0)+beta6*(pred1-pred0)*a1+beta7*a1*(w1-w0))
    
    INT.med.am <- ((a1-a0)*(beta2+beta6*(pred1)+beta7*w0))-(((a1-a0)*(beta2+beta6*(pred0)+beta7*(w0))))
    
    INT.med.aw <- ((a1-a0)*(beta2+beta6*(pred0)+beta7*w1))-(((a1-a0)*(beta2+beta6*(pred0)+beta7*(w0))))
    
    INT.med.amw <- ((a1-a0)*(beta2+beta6*(pred1)+beta7*w1))-
      ((a1-a0)*(beta2+beta6*(pred1)+beta7*w0))-
      ((a1-a0)*(beta2+beta6*(pred0)+beta7*w1))+
      (((a1-a0)*(beta2+beta6*(pred0)+beta7*(w0))))
    
    INT.med <- INT.med.am+INT.med.aw+INT.med.amw
    
    
    PIE.m <- (pred1-pred0)*(beta4+beta6*(a0))
    PIE.w <- (w1-w0)*(beta5+beta7*(a0))
    PIE.mw <- (beta4*(pred1-pred0)+beta5*(w1-w0)+beta6*a0*(pred1-pred0)+beta7*a0*(w1-w0)-PIE.m-PIE.w)
    ...
    ##  Y(a,m*,W(a*)) Y(a*,m*,W(a*))
    Y100. <- Y.fun(1,L.fun(C,1),C,0,w0)
    Y000. <- Y.fun(0,L.fun(C,0),C,0,w0)
    ##  Y(a,M(a*),W(a*)) Y(a*,M(a*),W(a*))
    Y10.0. <- Y.fun(1,L.fun(C,1),C,pred0,w0)
    Y00.0. <- Y.fun(0,L.fun(C,0),C,pred0,w0)
    ##  Y(a,M(a),W(a*))  Y(a*,M(a),W(a*))
    Y11.0. <- Y.fun(1,L.fun(C,1),C,pred1,w0)
    Y01.0. <- Y.fun(0,L.fun(C,0),C,pred1,w0)
    ##  Y(a,M(a*),W(a))  Y(a*,M(a*),W(a))
    Y10.1. <- Y.fun(1,L.fun(C,1),C,pred0,w1)
    Y00.1. <- Y.fun(0,L.fun(C,0),C,pred0,w1)
    ##  Y(a,M(a),W(a))   Y(a*,M(a),W(a))
    Y11.1. <- Y.fun(1,L.fun(C,1),C,pred1,w1)
    Y01.1. <- Y.fun(0,L.fun(C,0),C,pred1,w1)
    ...
    NDE.real <-INT.re.am.real+INT.re.aw.real+INT.re.amw.real+CDE.real
    #INT.ref.real <- INT.re.am.real+INT.re.aw.real+INT.re.amw.real
    ...
    if (bootstrap == TRUE){
      ...
      for (k in seq(1, iterations))
      {
        ...
        holder[k,"NDE"] <-  ((a1-a0)*(beta2+beta6*(pred0)+beta7*w0))
        
        holder[k,"CDE"] <-  ((a1-a0)*(beta2+beta6*0+beta7*0))
        
        holder[k,"INT.re.am"] <- ((a1-a0)*(beta2+beta6*(pred0)+beta7*0))-((a1-a0)*(beta2+beta6*0+beta7*0))
        holder[k,"INT.re.aw"] <- ((a1-a0)*(beta2+beta6*0+beta7*w0))-((a1-a0)*(beta2+beta6*0+beta7*0))
        holder[k,"INT.re.amw"] <- (((a1-a0)*(beta2+beta6*(pred0)+beta7*(w0))))-
          ((a1-a0)*(beta2+beta6*(pred0)+beta7*0))-
          ((a1-a0)*(beta2+beta6*0+beta7*w0))+
          ((a1-a0)*(beta2+beta6*0+beta7*0))
        ...
        holder[k,"NIE"] <- (beta4*(pred1-pred0)+beta5*(w1-w0)+beta6*(pred1-pred0)*a1+beta7*a1*(w1-w0))
        
        holder[k,"INT.med.am"] <- ((a1-a0)*(beta2+beta6*(pred1)+beta7*w0))-(((a1-a0)*(beta2+beta6*(pred0)+beta7*(w0))))
        holder[k,"INT.med.aw"] <- ((a1-a0)*(beta2+beta6*(pred0)+beta7*w1))-(((a1-a0)*(beta2+beta6*(pred0)+beta7*(w0))))

        holder[k,"INT.med.amw"] <- ((a1-a0)*(beta2+beta6*(pred1)+beta7*w1))-
          ((a1-a0)*(beta2+beta6*(pred1)+beta7*w0))-
          ((a1-a0)*(beta2+beta6*(pred0)+beta7*w1))+
          (((a1-a0)*(beta2+beta6*(pred0)+beta7*(w0))))
        ...
        holder[k,"PIE.m"] <- (pred1-pred0)*(beta4+beta6*(a0))
        holder[k,"PIE.w"] <-  (w1-w0)*(beta5+beta7*(a0))
        holder[k,"PIE.mw"] <- (beta4*(pred1-pred0)+beta5*(w1-w0)+beta6*a0*(pred1-pred0)+beta7*a0*(w1-w0)-PIE.m-PIE.w)-holder[k,"PIE.m"]-holder[k,"PIE.w"]
...
sim1 <- decompz(treatment, pre_cov, zmodels, y_form, m_form,
                m1.family = m1.family,
                m2.family = m2.family,
                l.family =  l.family ,
                y.family =  y.family ,
...
write.table(sim1,paste0(path_data,"sim3.csv"),sep = ",",col.names = TRUE,row.names = FALSE)
...
#   C <- rnorm(n) 
#   A<-rbinom(n,1,prob=0.5)
#   L<-rnorm(n,mean=(0.5+(0.37-eta*C)*C+(alpha*C+1.3)*A),sd=0.5) 
#   M<-rnorm(n,mean=(0.53*L+(0.35+alpha*C)*A+(0.26-eta*C)*C-0.1),sd=0.5)
#   W<-rnorm(n,mean=((0.67-eta*C)*C+0.25*L+(0.25+alpha*C)*A+0.25*M),sd=0.5) 
#   Y<-rnorm(n,mean=(0.122+(0.45+alpha*C)*A+(0.35+lambda*C)*L+(0.5-eta*C)*C+(0.45+gamma*C)*W+(0.25+gamma*C)*M+0.1*A*M+0.1*A*W+0.1*M*W+0.5*A*M*W),sd=0.5)
#   data <-as.data.frame(cbind(A=A,Y=Y,C=C,M=M,W=W,L=L))
#   
#   #抽象函数
#   C <- rnorm(n) 
#   A<-rbinom(n,1,prob=0.5)
#   L.fun <- function(C,A)rnorm(n,mean=(0.5+(0.37-eta*C)*C+(alpha*C+1.3)*A),sd=0.5) 
#   M.fun <- function(A,L,C)rnorm(n,mean=(0.53*L+(0.35+alpha*C)*A+(0.26-eta*C)*C-0.1),sd=0.5)
#   W.fun <- function(A,L,C,M)rnorm(n,mean=((0.67-eta*C)*C+0.25*L+(0.25+alpha*C)*A+0.25*M),sd=0.5) 
#   Y.fun <- function(A,L,C,M,W)rnorm(n,mean=(0.122+(0.45+alpha*C)*A+(0.35+lambda*C)*L+(0.5-eta*C)*C+(0.45+gamma*C)*W+(0.25+gamma*C)*M+0.1*A*M+0.1*A*W+0.1*M*W+0.5*A*M*W),sd=0.5)
#   
#   #alpha is a parameter that controls the degree to which C modifies the effects of A on L, M,W and Y
#   #gamma is a parameter that controls the degree to which C modifies the effect of M W on Y
#   #lambda s a parameter that controls the degree to which C modifies the effect of L on Y
...
#                                  bootstrap = TRUE, iterations = 1000,
#                                  data=data)
#   write.table(alpha_outcome[[g]],paste0(path_data,"alpha_outcome",alpha1[g],".csv"),sep = ",",col.names = TRUE,row.names = FALSE)
#   
# }
# 
# #over####
...
#                                  bootstrap = TRUE, iterations = 1000,
#                                  data=data)
#   write.table(gamma_outcome[[g]],paste0(path_data,"gamma_outcome",gamma1[g],".csv"),sep = ",",col.names = TRUE,row.names = FALSE)
#   
# }
# #over####
# 
...
#                                   bootstrap = TRUE, iterations = 1000,
#                                   data=data)
#   write.table(lambda_outcome[[g]],paste0(path_data,"lambda_outcome",lambda1[g],".csv"),sep = ",",col.names = TRUE,row.names = FALSE)
#   
# }
# #over####
# print("lambda")
...
#                                bootstrap = TRUE, iterations = 1000,
#                                data=data)
#   write.table(eta_outcome[[g]],paste0(path_data,"eta_outcome",eta1[g],".csv"),sep = ",",col.names = TRUE,row.names = FALSE)
#   
# }
# #over####
# 
...
# 
# 
# final <- cbind(sim1,alpha,gamma,lambda,eta)
# 
# write.table(final,paste0(path_data,"final.csv"),sep = ",",col.names = TRUE,row.names = FALSE)
# 
# 
# 
# 
