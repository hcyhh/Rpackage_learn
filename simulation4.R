...
M<-rbinom(n,size=1,expit(x=0.53*L+0.35*A+0.26*C-0.1))
#W<-rnorm(n,mean=(0.67*C+0.25*L+0.25*A),sd=0.5)
W<-rbinom(n,size=1,expit(x=-0.67*C+0.25*L+0.25*A))
...
M.fun <- function(A,L,C) expit(0.53*L+0.35*A+0.26*C-0.1)
W.fun <- function(A,L,C,M) expit(0.67*C+0.25*L+0.25*A)
...
write.table(sim1,paste0(path_data,"sim4.csv"),sep = ",",col.names = TRUE,row.names = FALSE)
